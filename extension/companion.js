...
(() => {
  ...
  function findByLabel(controls, pattern) {
    const re = pattern instanceof RegExp ? pattern : new RegExp(escRe(pattern), 'i');
    return controls.filter(({ label }) => label && re.test(label)).map(({ el }) => el);
  }
  ...
  async function fillCombobox(el, value) {
    try {
      ...
      const target = clean(value).toLowerCase();
      const match = options.find((o) => clean(o.textContent).toLowerCase() === target)
  ...
  // answer, scoped (when possible) to a container mentioning the question.
  function fillChoice(question, answer) {
    ...
    const candidates = [...document.querySelectorAll('input[type="radio"], input[type="checkbox"]')]
      .filter((el) => answerRe.test(labelFor(el)) || answerRe.test(clean(el.closest('label')?.textContent)));
  ...
  // ── fill engine ───────────────────────────────────────────────────────────
  // Returns [{key, label, status: 'filled'|'already'|'failed'|'notfound'}] so
  // the panel can pre-complete its checklist. Runs in every frame; the panel
  // merges (filled in any frame wins).
  async function runFill(plan, { waitMs = 1500, strict = false, topHost = '' } = {}) {
    ...
    const state = (r) => (r === 'already' ? 'already' : 'filled');
    ...
    const noteFilled = (el, key, value, result) => {
      ...
      else filledValues.set(el, { key, value: String(value) });
    };
    ...
    for (const field of plan.fields || []) {
      ...
      for (const sel of (pack && pack.profile[field.id]) || []) {
        ...
        if (el) { const r = fillControl(el, field.value); if (r) { noteFilled(el, field.id, field.value, r); push(`field:${field.id}`, field.id, r === 'already' ? 'already' : 'filled'); done = true; break; } }
      }
      if (!done) {
        for (const key of field.keys) {
          for (const el of findByLabel(controls, key)) {
            const r = fillControl(el, field.value);
            if (r) { noteFilled(el, field.id, field.value, r); push(`field:${field.id}`, field.id, r === 'already' ? 'already' : 'filled'); done = true; break; }
          }
          if (done) break;
        }
      }
    ...
    for (const entry of plan.learned || []) {
      ...
      for (const key of entry.keys || []) {
        for (const el of findByLabel(controls, key)) {
          ...
          if (r) { noteFilled(el, entry.key, value, r); push(`learned:${entry.key}`, entry.key, r === 'already' ? 'already' : 'filled', value); done = true; break; }
        }
        if (done) break;
      }
    ...
    for (const canned of plan.canned || []) {
      ...
      const re = new RegExp((canned.match || []).map(escRe).join('|'), 'i');
      let ok = false;
  ...
  let activeTab = 'fill'; // 'fill' | 'message' — the panel body shows one at a time
  ...
  async function loadPlan(itemId = null) {
    ...
    const res = await send({ type: 'companion:getPlan', url: location.href, item: itemId });
    if (!res || !res.ok) {
  ...
  panelBus['companion:autorun'] = async () => {
    if (running) return;
    if (!host) buildPanel();
    await loadPlan();
  ...
  // The one-click path: fill fields + answers, attach the resume, draft the
  // leftovers. Auto-run calls this on a URL match; the ⚡ button is the manual
  // trigger for the exact same flow. Nothing here submits.
  async function fillEverything({ auto = false } = {}) {
    ...
    try {
      ...
      const settings = await send({ type: 'companion:getSettings' });
      if (settings && settings.autoDraft) await autoDraftOpenQuestions({ waitMs: auto ? 9000 : 3000 });
    } finally {
      running = false;
    }
  }
  ...
  // ── rendering ─────────────────────────────────────────────────────────────
  const DOT = { filled: '#26a269', already: '#26a269', failed: '#e5a50a', notfound: '#e5a50a' };
    ...
    for (const row of root.querySelectorAll('.co-row')) {
      ...
      dot.title = status === 'filled' ? 'auto-filled' : status === 'already' ? 'was already filled'
        : status === 'failed' ? 'auto-fill failed — use Insert or Copy'
          : status === 'notfound' ? 'field not found on this page — use Insert or Copy' : 'pending';
    }
  }

  function renderRows() {
    if (activeTab !== 'fill') return; // the message tab owns the body right now
    ...
    if (plan.mode === 'profile' && plan.candidates && plan.candidates.length) {
      ...
      pick.className = 'co-pick';
      pick.innerHTML = '<option value="">— link this page to a pipeline item —</option>'
        + plan.candidates.map((c) => `<option value="${escHtml(c.id)}">${escHtml(clean(c.company))} — ${escHtml(clean(c.role))} (${escHtml(c.status)})</option>`).join('');
      pick.addEventListener('change', () => { if (pick.value) loadPlan(pick.value); });
    ...
    // AI assist: read the focused question (or type one), draft server-side
    // from the candidate's own material, review/edit, insert.
    section('AI assist');
    const assist = document.createElement('div');
    assist.className = 'co-assist';
    assist.innerHTML = `
    ...
    root.getElementById('co-assist-read').addEventListener('mousedown', (e) => e.preventDefault());
    ...
    root.getElementById('co-assist-ins').addEventListener('mousedown', (e) => e.preventDefault());
  ...
  function buildPanel() {
    ...
    root.innerHTML = `
      <style>
        ...
        * { box-sizing: border-box; font-family: system-ui, -apple-system, sans-serif; }
        ...
        .co-assist textarea { width: 100%; margin-top: 6px; padding: 6px; background: var(--co-surface); color: var(--co-fg);
      ...
      <div class="co-panel">
        ...
        <div class="co-tabs">
          <button class="co-tab on" data-tab="fill" title="Fill this application form from your pipeline">Fill</button>
        ...
        <div class="co-actions" id="co-actions">
          <button class="co-btn co-primary" id="co-autofill" title="Fill contact fields and answers, attach the resume, and draft any open questions — whatever fails stays in the list below. Never submits.">⚡ Fill everything</button>
    ...
    root.getElementById('co-close').addEventListener('click', () => { host.remove(); host = null; });
