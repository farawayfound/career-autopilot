 ...
 * scripts/insights/company-history.mjs. `flagValue`/`hasFlag` are the shared fix.
 *
 ...
 * fixed independently, in the identical shape, in scripts/scan/scan-ats-full.mjs
 * (#1633/#1635), scripts/insights/reply-watch.mjs (#2743/#2745) and scripts/tracker/dedup-tracker.mjs
 ...
 *
 * Three scripts hand-rolled this exact shape independently — scripts/scan/scan-ats-full.mjs
 * (#1633/#1635), scripts/insights/reply-watch.mjs (#2743/#2745), scripts/tracker/dedup-tracker.mjs
 ...
 * the next script does not have to rediscover it; scripts/scan/scan.mjs's own instance
...
export function validateFlags(args, knownFlags, usage, { valueFlags = [], requireOperand = false } = {}) {
  ...
  // it happens to start with `-`. Mirrors scripts/scan/scan-ats-full.mjs's own adjacency
  ...
  // themselves and say more than a generic message can. scripts/scan/scan.mjs's requireValue()
  ...
  if (missingOperand.length > 0) {
    // Wording matches scripts/scan/scan.mjs's own hand-rolled requireValue() (scripts/scan/scan.mjs:2345),
 ...
 *
 * scripts/scan/detect-reposts.mjs derived this and wrote down why (#2702); it is exported
 ...
 * scripts/scan/detect-reposts.mjs documents: an unusable value is treated as if the flag
