...
// Moved verbatim from tests/test-all.mjs (issue #1440); no framework by design:
// the suite must run on a fresh clone with only Node.
import { execFileSync } from 'child_process';
import { cpSync, existsSync, mkdirSync, readdirSync, rmSync as _rmSync, symlinkSync, copyFileSync, readFileSync } from 'fs';
...
// wrapper tests/test-all.mjs applies to its own call sites (#3066), shared so the
 ...
 * Moved verbatim from the tail of tests/test-all.mjs — output must stay byte-identical.
 */
 ...
 *
 * That is not hypothetical: `tests/agent-inbox-tests.mjs` grew past this cap, and a
 ...
 * property tests/test-all.mjs's own `tests/` discovery relies on (#1440).
 *
 ...
 * scripts/cv/generate-pdf.mjs, whose siblings lib/theme-style.mjs and lib/tracker-utils.mjs both
...
export function hermeticGitEnv(gitConfigPath, base = process.env) {
  ...
  //                          writes. The fixtures in tests/test-all.mjs call `git config` to
...
/**
 * Copy a script and its whole relative-import closure into a sandbox,
 * preserving repo-relative paths — so `scripts/insights/contacts.mjs` lands at
 * `<sandbox>/scripts/insights/contacts.mjs` next to a `<sandbox>/lib/` holding
 * everything it imports, and `REPO_ROOT` inside the sandbox is the sandbox.
 *
 * Before the 2.0.0 layout every script sat at the root and a test could copy
 * it alone into a temp dir; now each one imports `../../lib/repo-root.mjs` and
 * a handful of siblings, and a copy that forgets one dies on
 * ERR_MODULE_NOT_FOUND before the assertion runs. The closure is walked from
 * the source text (static `import … from`, side-effect imports, dynamic
 * `import('…')` literals, and `new URL('…', import.meta.url)` sidecars such as
 * `lib/tracker-aliases.json`), which is the same walk the deploy guards use.
 *
 * @param {string} sandboxRoot - Existing directory that plays the repo root.
 * @param {string} relScript - Repo-relative path, e.g. `scripts/cv/generate-pdf.mjs`.
 * @param {{extra?: string[]}} [opts] - More repo-relative files to stage verbatim.
 * @returns {string} Absolute path of the staged script.
 */
export function stageScript(sandboxRoot, relScript, { extra = [] } = {}) {
  const seen = new Set();
  const queue = [relScript.split(/[\\/]/).join('/'), ...extra];
  const SPEC = /(?:\bfrom\s*|^[ \t]*import\s*|\bimport\s*\(\s*|new URL\(\s*)['"]((?:\.\.?\/)[^'"\n]+)['"]/gm;
  while (queue.length) {
    ...
    const dest = join(sandboxRoot, ...rel.split('/'));
    mkdirSync(dirname(dest), { recursive: true });
  ...
  return join(sandboxRoot, ...relScript.split(/[\\/]/));
}
